...
impl DatasetCollectorBuilder {
    ...
    pub fn get_collection(mut self) -> Result<DatasetCollector> {
        if let Some(ref name) = self.from_dataset {
            let cname = CString::new(name.as_bytes())?;

            unsafe {
                let handle = super::ZfsDatasetHandle::new(cname)?;

                if handle.r#type() != libzfs::zfs_type_t::ZFS_TYPE_VOLUME {
                    sys::zfs_iter_filesystems(
                        handle.handle,
                        Some(zfs_list_cb),
                        &mut self as *mut _ as *mut libc::c_void,
                    );
                }
            }
        } else {
        ...
        Ok(DatasetCollector::new(self.datasets))
    }
}
